...
var cloudant = {
    url : "https://e382abec-33ed-4324-b26f-b1e1157f9908-bluemix:05ef7216c9d22243232f69bb4736e8265d218b56c32f80a271e16a3cea11e7ae@e382abec-33ed-4324-b26f-b1e1157f9908-bluemix.cloudant.com"
};
...
var db = nano.db.use('weather_data');
    ...
    if (!cancelling) {
      ...
      if (payload.eventType == 'MaxMin Temp') {
        ...
        events.push(new Event(eventCount, payload));
          ...
          var scoreRecord = { 'number': eventCount, 'eventTime' : payload.eventTime, 'eventType': payload.eventType, 'eventStation' : payload.eventStation };
          ...
          db.insert(scoreRecord, function(err, body, header) {
              ...
              var config = {
                  ...
                  tenantId: '9ad471c34677492ca4b021971eb103ba',    //projectId from credentials
                  domainId: 'd11b0f20a4d344598e988f8f19f81624',
                  username: 'admin_e090d78f87f6b84ab70e0af345f8d1ec97348fe5',
                  password: 'ahoe]yEx8S6fy.E8',
              ...
              storageClient.createContainer({
                  ...
                  if (err) {
                      ...
                      if (true){
                          ...
                          var writeStream = fs.createWriteStream("weather.txt");
                          writeStream.write(eventCount + "," + payload.eventTime + "," + payload.eventType + "," + payload.eventStation);
                          writeStream.end();



                          var myFile = fs.createReadStream('weather.txt');

                          var upload = storageClient.upload({
                              container: container.name,
                              remote: 'data' + eventCount
                          });
